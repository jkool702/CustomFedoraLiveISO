...
# # # # # # # # # # # # # # SET PARAMETERS  # # # # # # # # # # # # # # # # # 
# all paramaters (except for customIsoUSBDevPath) will be assigned default values if left blank

#=37							# Fedora release version to use. Default is to try to extract it from the base iso filename.
...
#rootfsSizeGB=32 								# this is how much space youll have in the live OS. Because this is in a squashfs increasing the free space doesnt really increase the ISO size much. Default is 32 GB.customIsoReleaseVer
...
# TO USE PRE-DOWNLOADED IMAGE, SPECIFY origIsoSource="file://<path>"
# if left blank, you will be promoted to choose one of the current fedora "respins" to download
...
customIso_nspawnRootfs() {    
    ...
    # USE `systemd-nspawn -b -D` TO CUSTOMIZE THE SYSTEM
    # NOTE: you may need to pass --network-interface=<...> to get internet in the container, at the
    #       expense of losing internet everywhere else in the system. Determine the <...> with `ifconfig`
    ...
    # calls to try if you dont have internet in the systemd-nspawn container
    #systemd-nspawn -b -D "${customIsoRootfsMountPoint}"
    #systemd-nspawn -n -b -D "${customIsoRootfsMountPoint}"
    #systemd-nspawn --network-interface=<...> -b -D "${customIsoRootfsMountPoint}"
    
    
    # When done, press `<ctrl>` + `]` 3 times in a row in quick succession
...
customIso_mockBuildAnacondaBootIso() {   
    # build anaconda boot.iso with lorax in mock
    #mock --enable-network -r fedora-${customIsoReleaseVer}-$(uname -m) --shell -- /bin/bash -c 'cd "'"${customIsoTmpDir}"'/lorax/" && PATH="'"${customIsoTmpDir}"'/lorax/src/sbin/":"$PATH" PYTHONPATH="'"${customIsoTmpDir}"'/lorax/src/" "'"${customIsoTmpDir}"'/lorax/src/sbin/lorax" -p Fedora -v '${customIsoReleaseVer}' -r '${customIsoReleaseVer}' -s https://dl.fedoraproject.org/pub/fedora/linux/releases/'${customIsoReleaseVer}'/Everything/x86_64/os/ -s https://dl.fedoraproject.org/pub/fedora/linux/updates/'${customIsoReleaseVer}'/Everything/x86_64/  "'"${customIsoTmpDir}"'/lorax//anaconda_iso/"'
